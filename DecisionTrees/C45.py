...
class C45(Tree.Tree):
    """
    This class implements C4.5 algorithm to create decision trees from a training dataset.
    """
    def __init__(self, dataset, attributeClass = 'CLASS'):
        """
        Creates an instance of C45 class, creating the associated decision tree.
    ...
    def _getGainRatio (self, dataset, attributeName):
        """
        Computes the Gain Ratio associated to one attribute in a dataset
        :param dataset: training element dataset in DataFrame format (pandas)
        :param attributeName: name of the attribute to be used to filter the dataset and to compute its Gain Ratio
    ...
    def _getNextAttribute(self, dataset, attributeClass = 'CLASS'):
        """
        Returns the next attribute to split the data according to the greatest Gain Ratio
        :param dataset: training element dataset in DataFrame format (pandas)
        :param attributeClass: name of the attribute which provides the class of the elements. By default the attribute will be 'CLASS'
        :return: the attribute with better Gain Ratio.
        """
        entropies = {}
        for attribute in dataset.keys():
            if attribute == attributeClass:
                continue
            entropies[attribute] = self._getGainRatio(dataset, attribute)
    ...
    def _createTree(self, dataset, attributeClass = 'CLASS', parent = None):
        """
        Creates the decision tree based on C4.5 criteria
        :param dataset: training element dataset in DataFrame format (pandas)
        :param attributeClass: name of the attribute which provides the class of the elements. By default the attribute will be 'CLASS'
        ...
        if not parent:
            parent = {}
        attribute, gainRatio = self._getNextAttribute(dataset)
...
if __name__ == '__main__':
    ...
    for element in elements:
        classification = c45.classify(element)
        if element['CLASS'] == classification:
            hits += 1
    import pprint
    pprint.pprint (c45.getTree())
    print("Total Depth: %s" % c45.getTreeDepth())
